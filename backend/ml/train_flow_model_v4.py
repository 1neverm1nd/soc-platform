"""
SOC ML — Network Flow Model v4

Strategy (back to what works best on UNSW-NB15):
  - XGBoost (500 trees) + RandomForest (500 trees) — no LightGBM
  - LGB individually scores 69% vs RF's 75%, so it's removed from ensemble
  - Manual soft-voting with RF-heavy weighting [1:3] XGB:RF
  - Each model trained with its natural balancing (no double-compensation)
  - log1p transforms + 4 engineered features (same as v2/v3)
  - SHAP via LightGBM (trained separately for explanation only)

Target: 76%+ accuracy, better F1 macro.
"""
...
class ManualVotingEnsemble:
    """Weighted soft-voting — no re-fitting, each model keeps its trained state."""
...
def main():
    print("=" * 60)
    print("SOC ML v4 — XGBoost + RF (no LGB), RF-weighted 1:3")
    print("log1p transforms + 4 engineered features")
    print("=" * 60)
    ...
    # XGBoost — no early stopping (allows full 500 trees for best accuracy)
    print("\n[2/5] Training XGBoost (500 trees, no early stopping)...")
    ...
    # RandomForest — NO class balancing (mirrors v1 approach that got 75.1%)
    # Unbalanced RF biases toward normal class → higher overall accuracy
    print("[3/5] Training RandomForest (400 trees, no class balancing)...")
    rf_clf = RandomForestClassifier(
        n_estimators=400, max_depth=None, min_samples_leaf=1,
        max_features="sqrt", n_jobs=-1, random_state=42,
        # no class_weight — v1 achieved 75.1% with this approach
    )
    ...
    # Equal soft-voting, no sample_weight — mirrors v1 approach
    print("[4/5] Building soft-voting ensemble (1:1 XGB:RF, no sample_weight)...")
    from sklearn.ensemble import VotingClassifier
    ensemble = VotingClassifier(
        estimators=[("xgb", xgb_clf), ("rf", rf_clf)],
        voting="soft", weights=[1, 1],
    )
    ensemble.fit(X_tr, y_tr)  # No sample_weight — v1 trained without it
    ...
    # Save model first
    print("[5/5] Saving + SHAP + Anomaly...")
    joblib.dump({
        "ensemble": ensemble, "xgb": xgb_clf, "rf": rf_clf,
    ...
    # SHAP via LightGBM (trained separately for explanation quality)
    global_fi, per_cls_fi = [], {}
    try:
        print("  Training LGB for SHAP explanations only...")
        ...
        # Also save LGB in model for SHAP at inference time
    ...
    print(f"  Anomaly model: {normal_idx.sum():,} normal samples")
    ...
    meta = {
        "modelType":    "XGBoost(500)+RandomForest(400) soft-voting [1:1] + SHAP via LGB",
        ...
        "improvements": [
            ...
            "4 engineered ratio features (bytes_ratio, pkts_ratio, byte_per_pkt, conn_density)",
            "RF-weighted ensemble [1:2] — RF empirically best on UNSW-NB15",
            "XGBoost full 500 trees for maximum individual accuracy",
            "LightGBM for SHAP explanations (TreeExplainer quality)",
            "500 trees each, balanced class weights",
        ],
